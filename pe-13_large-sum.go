...
func main() {
	...
	stringNumbers, err := ReadNumbersFromFile(fileName)
	if err != nil {
		panic(err)
	}
...
func DisplayResults(summedNumbers []uint8) {
	...
	if l > 10 {
		...
		fmt.Println("first 10   |  rest of number")
		fmt.Println(strings.Repeat("-", 55))
		fmt.Println(Stringify(summedNumbers[l-10:]), "|", Stringify(summedNumbers[:l-10]))
	} else {
		fmt.Println("Sum of numbers from", fileName, ":")
		fmt.Println(Stringify(summedNumbers))
	}
...
func ReadNumbersFromFile(filename string) ([]string, error) {
	...
	for err != io.EOF {
		...
		line = strings.Trim(line, string(delim))
	...
	l := len(stringArray)
	if stringArray[l-2] == "" {
		returnSlice = stringArray[:l-2]
	} else {
		returnSlice = stringArray[:l-1]
	}
...
//         Name:  ReadNumbersFromFile
//  Description:  Fills the numbers array
// =============================================================================
